...
function fixture(t){const root=fs.mkdtempSync(path.join(os.tmpdir(),'wickrun-file-backup-test-'));t.after(()=>{assert.equal(path.dirname(root),fs.realpathSync(os.tmpdir()));assert.ok(path.basename(root).startsWith('wickrun-file-backup-test-'));fs.rmSync(root,{recursive:true,force:true});});const userData=path.join(root,'app'),source=path.join(root,'source');fs.mkdirSync(userData);fs.mkdirSync(source);fs.writeFileSync(path.join(source,'report.txt'),'original');fs.writeFileSync(path.join(userData,'store.json'),JSON.stringify({kv:{},secrets:{}}));const files=createTeamFiles(userData),session=files.create({projectId:'p',taskId:'r',memberId:'m',root:source},[source]);fs.writeFileSync(path.join(session.isolatedRoot,'report.txt'),'draft');files.diff(session.id);return {root,userData,source,session};}
